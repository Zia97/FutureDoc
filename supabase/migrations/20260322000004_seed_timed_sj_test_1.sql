-- Seed: Timed SJ Test 1 (test_id = 1)
...
BEGIN
  ...
  INSERT INTO timed_situational_judgement_questions (scenario_id, label_set, question_text, correct_answer, answer_reason, order_index) VALUES
    (v_s06, 1, 'How important is it to address Dr. Turner''s offensive joke rather than simply letting it pass?', 'Important', 'This is important because discriminatory or offensive behaviour should not be normalised, even in social settings.', 0),
  ...
  INSERT INTO timed_situational_judgement_questions (scenario_id, label_set, question_text, correct_answer, answer_reason, order_index) VALUES
    ...
    (v_s11, 2, 'Alex Johnson confronts Dr. Davis publicly in the middle of the workplace to make it clear that the comments are unacceptable.', 'Appropriate, but not ideal', 'This is appropriate, but not ideal because challenging the behaviour may be justified, but doing so publicly may be less effective and more disruptive than seeking support or addressing it through appropriate channels.', 2);
  ...
  INSERT INTO timed_situational_judgement_questions (scenario_id, label_set, question_text, correct_answer, answer_reason, order_index) VALUES
    ...
    (v_s15, 1, 'How important is it to notify the patient''s family before the patient has been assessed?', 'Not important at all', 'This is not important at all at this stage because the immediate priority is to assess the patient, start appropriate management, and seek senior help if needed.', 1),
  ...
  INSERT INTO timed_situational_judgement_questions (scenario_id, label_set, question_text, correct_answer, answer_reason, order_index) VALUES
    ...
    (v_s17, 2, 'Pausing the prescription and asking Dr. Sarah Rodriguez or another appropriate senior clinician to review it before it is written up.', 'A very appropriate thing to do', 'This is a very appropriate thing to do because it escalates the concern appropriately and helps prevent a possible medication error.', 2);
  ...
  INSERT INTO timed_situational_judgement_questions (scenario_id, label_set, question_text, correct_answer, answer_reason, order_index) VALUES
    (v_s19, 2, 'Confront Dr. Patel directly and ask for an explanation before deciding what further action is needed.', 'A very appropriate thing to do', 'This is a very appropriate thing to do because it addresses the concern fairly and allows clarification before further escalation.', 0),
  ...
  INSERT INTO timed_situational_judgement_questions (scenario_id, label_set, question_text, correct_answer, answer_reason, order_index) VALUES
    ...
    (v_s22, 2, 'Ask the patient whether they know about the medication error and proceed depending on what they say.', 'A very inappropriate thing to do', 'This is a very inappropriate thing to do because resolving a medication error is the clinician''s responsibility and should not be left to the patient.', 2);
