...
// Friendly labels for the stimulus types stored in
// timed_quantitative_reasoning_sets.stimulus->>'type'.
...
function aggregate(rows, tests) {
  ...
  // QR test.id is the string "timed-qr-test-N" while attempt.test_id is
  // the SMALLINT N. Build a numeric → test lookup so the completion row
  // can pull the right questionCount.
  ...
  const trend = rows.map((r) => ({
    ...
    score: getQRScaledScore(r.score_percent),
  ...
  // Difficulty + stimulus type + completion + timing all walk the same answer rows.
  ...
  for (const attempt of rows) {
    ...
    for (const ans of attempt.answers) {
      ...
      const stype = ans.stimulus_type ?? 'unknown';
      if (!stimulusTotals[stype]) stimulusTotals[stype] = { correct: 0, total: 0 };
      stimulusTotals[stype].total++;
      if (isCorrect) stimulusTotals[stype].correct++;
  ...
  // Stimulus rows sorted by sample size descending so the most-sat
  // stimulus types come first. Types the user hasn't seen yet are excluded.
  const stimulus = Object.entries(stimulusTotals)
    .map(([key, v]) => ({
      key,
      label: STIMULUS_LABELS[key] ?? key,
  ...
  // metadata (26 min / 36 q ≈ 43s for QR). Used as a benchmark.
...
// UCAT scaled score line chart. Y-axis fixed to 300–900. Each data point
// is coloured by its scoreColor band, and a dashed line shows the 2025 UK
// QR mean (661). Vertical bars on each point show the ±40 SEM.
...
export default function QRAnalyticsScreen({ route }) {
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={[styles.benchmarkStrip, { backgroundColor: t.bgCard, borderColor: t.border }]}>
          ...
          <Text style={[styles.benchmarkValue, { color: scoreColor(stats.headline.avg, t) }]}>
            ~{benchmarkDelta(stats.headline.avg, QR_2025_MEAN)}
          </Text>
        </View>
        ...
        >
          ...
          <Text style={[styles.cardSub, { color: t.textSecondary }]}>
            ...
            estimate. Dashed line is the 2025 UK average ({QR_2025_MEAN}).
          </Text>
          ...
          {(() => {
            ...
            const percentileText = latest ? formatPercentile('qr', latest.score) : null;
        ...
        <View style={[styles.card, { backgroundColor: t.bgCard, borderColor: t.border }]}>
          ...
          <Text style={[styles.cardSub, { color: t.textSecondary }]}>
            ...
            tables — see where your weak spots are.
          </Text>
          <View style={{ marginTop: 14 }}>
            {stats.stimulus.length === 0 ? (
              <Text style={[styles.emptyInline, { color: t.textSecondary }]}>
                No data yet.
              </Text>
            ) : (
              stats.stimulus.map((s) => (
                <StatBar
                  key={s.key}
                  label={s.label}
                  value={s.value}
                  sub={`${s.correct} / ${s.total}`}
                  color={s.value != null ? scoreColor(300 + (s.value / 100) * 600, t) : t.accent}
                  barBg={t.bgInput}
                  textColor={t.text}
...
// Calls out the user's strongest and weakest stimulus type when there's
// enough data to be meaningful. Requires ≥3 samples per type so a single
// lucky/unlucky question doesn't swing the call.
function stimulusInsight(stimulus) {
  const eligible = stimulus.filter((s) => s.total >= 3 && s.value != null);
