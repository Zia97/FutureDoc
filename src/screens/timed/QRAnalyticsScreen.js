...
function aggregate(rows, tests) {
  ...
  for (const attempt of rows) {
    ...
    // Merge stimulus breakdown
    if (s.stimulus_breakdown) {
      for (const [stype, counts] of Object.entries(s.stimulus_breakdown)) {
        if (!stimulusTotals[stype]) stimulusTotals[stype] = { correct: 0, total: 0 };
        stimulusTotals[stype].correct += counts.correct ?? 0;
        stimulusTotals[stype].total += counts.total ?? 0;
      }
    }
