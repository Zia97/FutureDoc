  ...
  return (
    ...
    >
      ...
      {completed ? (
        ...
        <Text style={[styles.demoNote, { color: colors.textMuted }]}>
          Read the stimulus, answer the question, then study the breakdown — that's where the method is taught.
        </Text>
      )}
    </View>
  );
  ...
  if (kind === 'stimulus') {
    return (
      <View
        key={`stimulus-${index}`}
        style={[
          index > 0 ? styles.blockSpacing : null,
          styles.stimulusCard,
          {
            backgroundColor: isDark ? 'rgba(8, 20, 38, 0.5)' : 'rgba(248, 252, 255, 0.92)',
            borderColor: hexToRgba(accent, 0.32),
          },
        ]}
      >
        {step.title ? (
          <Text style={[styles.stimulusTitle, { color: colors.text }]}>{step.title}</Text>
        ) : null}
        {step.body ? (
          <Text style={[styles.stimulusBody, { color: colors.textSecondary }]}>{step.body}</Text>
        ) : null}
        <View style={styles.stimulusRender}>
          <QRStimulusRenderer stimulus={step.stimulus} />
        </View>
      </View>
    );
  }
      ...
      <View style={styles.modalOverlay}>
        ...
        >
          ...
          <Text style={[styles.modalBody, { color: colors.textSecondary }]}>
            You've finished every QR lesson. Now put your skills to the test with practice questions or a timed exam.
          </Text>
...
export default function QuantitativeReasoningLessonScreen({ navigation, route }) {
  ...
  const launchTutorDemo = useCallback(() => {
    ...
    navigation.navigate('QRQuestion', { mode: 'tutorDemo' });
  }, [demoLaunchIndex, handleQuestionAnswered, navigation]);

  const launchWorkedExample = useCallback(() => {
    navigation.navigate('QRQuestion', { mode: 'workedExample', exampleId: lesson.id });
